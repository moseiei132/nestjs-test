...
describe('PostService', () => {
  ...
  describe('getPostsByTopicId', () => {
    ...
    it('shoud be throw error Posts not found', async () => {
      PostRepository.prototype.find = jest.fn().mockReturnValue([])
      await expect(postService.getPostsByTopicId(1)).rejects.toThrow(
        'Posts not found',
      )
    })
  })
  describe('getFirstPostByTopicId', () => {
    ...
    it('shoud be throw error Post not found', async () => {
      PostRepository.prototype.findOne = jest.fn()
      await expect(postService.getFirstPostByTopicId(1)).rejects.toThrow(
        'Post not found',
      )
    })
  })
  ...
  describe('deletePosts', () => {
    it('should be success', async () => {
      postService.getPostsByTopicId = jest
        .fn()
        .mockReturnValue([mockPost(), mockPost()])
    ...
    it('should be throw error Posts not found', async () => {
      ...
      await expect(postService.deletePosts(1)).rejects.toThrow(
        'Posts not found',
      )
    })
  })
  ...
  describe('editPost', () => {
    it('should be success', async () => {
      ...
      PostRepository.prototype.save = jest
        .fn()
        .mockReturnValue({ ...mockPostData, body: 'edit' })
    ...
    it('should be throw error Post not found', async () => {
      ...
      await expect(
        postService.editPost({ postId: 1, userId: 3, body: 'edit' }),
